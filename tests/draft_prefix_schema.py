...
Explicit behavioral changes (all flagged for review):
  ...
  4. "If new characters/groups/elements/events are identified" is canonicalized to
     "If new entries are identified" so the shared slab is contiguous.
  5. GroupMap gate-check "(all groups)" stays verbatim but moves to a trailing
...
def aperture_no_branch(orig, noun_phrase_end, list_clause):
    """Take the original aperture block and replace the '...added to the '<b>' list?' clause."""
    a = extract(orig, "In the latest exchange(s)", noun_phrase_end)
    return a + list_clause


# ===========================================================================
# TIER 1a — new_entry_query_prompt_template
# ===========================================================================
def discovery(defname, noun_phrase_end, list_clause, guidance_extract, example_anchor,
    ...
    bs = paragraphs(q)
    aperture = aperture_no_branch(q, noun_phrase_end, list_clause)
    guidance = None
    for p in bs:
    ...
    parts = [LEAD, aperture, RESP_ARRAY, IF_NEW_CANON, example, guidance]
...
NOUN_END = "mentioned or implied"
LIST_CLAUSE = " that should be added to the list?"

# CharacterMap
set_new("CharacterMap", "new_entry_query_prompt_template", discovery(
    "CharacterMap", NOUN_END, LIST_CLAUSE,
...
set_new("ElementMap", "new_entry_query_prompt_template", discovery(
    "ElementMap", NOUN_END, LIST_CLAUSE,
...
def discovery_lean(defname, noun_phrase_end, list_clause, guidance_anchor, example_anchor):
    q = t(defname, "new_entry_query_prompt_template")
    bs = paragraphs(q)
    aperture = aperture_no_branch(q, noun_phrase_end, list_clause)
    guidance = next(p for p in bs if guidance_anchor in p)
    example = next(p for p in bs if example_anchor in p)
    parts = [LEAD, aperture, RESP_ARRAY, IF_NEW_CANON, example, guidance, NAME_EXACTLY,
...
set_new("StoryEvents", "new_entry_query_prompt_template", discovery_lean(
    "StoryEvents",
    "new significant story events", " that should be added to the list?",
...
set_new("PastEvents", "new_entry_query_prompt_template", discovery_lean(
    "PastEvents",
    "new backstory scenes/events", " that should be added to the list?",
...
set_new("CrucialEvents", "new_entry_query_prompt_template", discovery_lean(
    "CrucialEvents",
    "new major story events", " that should be added to the list?",
    "Any significant events should be added to the list.",
    '["The Ambush at Dawn", "Discovery of the Old Bunker"]'))
...
# ===========================================================================
def gen_entry(defname, sentence, body_extracts, tail_extracts, tail_literals=()):
    """sentence: full first line (verbatim hand-pick). body/tail: (start,end) extracts."""
    q = t(defname, "new_entry_prompt_template")
    schema_block = "Relevant Schema:\n```json\n{{ schema_snippet }}\n```"
    example_block = "Example JSON structure:\n```json\n{{ example_json }}\n```"
    parts = [sentence if isinstance(sentence, str) else extract(q, sentence[0], sentence[1]),
             schema_block, example_block]
...
set_new("CharacterMap", "new_entry_prompt_template", gen_entry(
    "CharacterMap",
    ("Based on all of the information in the last scene, generate the full data for the new ",
     "character described here."),
    [("Please provide your response as a complete JSON object for this new character.", "this new character."),
     IMP_CHAR],
...
set_new("GroupMap", "new_entry_prompt_template", gen_entry(
    ...
    [("Please provide your response as a complete JSON object for this new group.", "this new group."),
     IMP_GROUP],
    [("The entry to populate: '{{ entry_name }}'", "{{ entry_name }}")]))
...
set_new("ElementMap", "new_entry_prompt_template", gen_entry(
    ...
    [("Please provide your response as a complete JSON object for this new element.", "this new element."),
...
for defname in ("StoryEvents", "CrucialEvents"):
    set_new(defname, "new_entry_prompt_template", gen_entry(
        ...
        [("Please provide your response as a complete JSON object for this new event.", "this new event."),
         IMP_EVENT],
        [("The entry to populate: '{{ entry_name }}'", "{{ entry_name }}")],
...
set_new("PastEvents", "new_entry_prompt_template", gen_entry(
    "PastEvents",
    ("Based on all of the information in the last scene, generate the full data for the new ",
     "backstory event described here."),
    [("Please provide your response as a complete JSON object for this new event.", "this new event."),
     IMP_EVENT],
    [("The entry to populate: '{{ entry_name }}'", "{{ entry_name }}")],
    ["Current message node: {{ current_message_node }}"]))
...
set_new("Scenes", "new_entry_prompt_template", gen_entry(
    ...
    [("Please provide your response as a complete JSON object for this new scene.", "this new scene."),
