...
class VRAMManager:
    def __init__(self, virtual_memory_dir: Optional[str] = None, max_vram_mb: Optional[int] = None):
    ...
    def _get_kv_cache(self, model) -> dict[str, torch.Tensor]:
        ...
        try:
            ...
            if hasattr(inner_model, '_ctx'):
                print(f"{_SUCCESS}Found LlamaCpp model{_RESET} {hasattr(inner_model, 'n_tokens')} {hasattr(inner_model, 'input_ids')} {hasattr(inner_model, 'scores')}")
                if hasattr(inner_model, 'n_tokens') and hasattr(inner_model, 'input_ids') and hasattr(inner_model, 'scores'):
                    ...
                    cache = {
                        ...
                        'input_ids': inner_model.input_ids.copy() if isinstance(inner_model.input_ids, (list, torch.Tensor)) else inner_model.input_ids,
                        'scores': inner_model.scores.copy() if isinstance(inner_model.scores, (list, torch.Tensor)) else inner_model.scores,
    ...
    def get_context_cache(self) -> Optional[Dict]:
    ...
    def _save_llamacpp_cache(self, model):
        ...
        cache = {
            ...
            'last_n_tokens_data': deep_copy_array(model.model.last_n_tokens_data) if hasattr(model.model, 'last_n_tokens_data') else None,
