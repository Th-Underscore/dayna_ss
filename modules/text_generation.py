...
def _generate_reply(question, state, stopping_strings=None, is_chat=False, escape_html=False, for_ui=False):
    ...
    for reply in generate_func(question, original_question, state, stopping_strings, is_chat=is_chat):
        ...
        try:
            reply = apply_extensions('output_stream', reply, state, is_chat=is_chat)
        except Exception:
            try:
    ...
    if not is_chat:
        # Call realtime stream hook once for the final (non-stream) output too
        try:
            reply = apply_extensions('stream', reply, state, is_chat=is_chat)
        except Exception:
            try:
                logger.error('Error in streaming extension hook')
            except Exception:
